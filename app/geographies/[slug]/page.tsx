...
const REGION_COLORS: Record<string, string> = {
  'Africa': '#D6A35C', 'Asia': '#5B5BD6', 'Europe': '#46B8D8',
  'Latin America': '#F26669', 'Middle East': '#3DD68C', 'North America': '#9AB654',
  'UAE': '#3DD68C', 'KSA': '#46B8D8', 'Global': '#9AB654',
};
const FALLBACKS = ['#3DD68C','#46B8D8','#5B5BD6','#D6A35C','#9AB654','#35B979','#F26669'];
function regionColor(name: string) {
  return REGION_COLORS[name] || FALLBACKS[name.charCodeAt(0) % FALLBACKS.length];
}

export default async function GeographyPage({ params }: { params: { slug: string } }) {
  const name = unslug(params.slug);
  const accent = regionColor(name);
        ...
        <div className="page-enter" style={{ padding: '28px 32px 40px' }}>
          ...
          <div style={{
            ...
            <div style={{ padding: '20px 24px 22px' }}>
              <p className="eyebrow" style={{ marginBottom: '4px' }}>Geography</p>
